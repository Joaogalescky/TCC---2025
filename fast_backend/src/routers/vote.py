...
@router.post('/election/{election_id}', status_code=HTTPStatus.CREATED, response_model=VoteResponse)
async def cast_vote(election_id: int, vote: VoteSchema, session: Session, current_user: CurrentUser):
    """Registra um voto criptografado usando sua abordagem original"""
    
    # Verificar se candidato existe e está na eleição
    ...
    # Verificar se usuário já votou nesta eleição
    existing_vote = await session.scalar(
        select(Vote_Election)
        .join(Election_Candidate, Vote_Election.fk_election_candidate == Election_Candidate.id)
        .where(
    ...
    if not tally:
        ...
        candidates_count = await session.scalar(
            select(func.count(Election_Candidate.id)).where(Election_Candidate.fk_election == election_id)
        )
    ...
    tally.encrypted_tally = crypto_service.add_vote_to_tally(tally.encrypted_tally, encrypted_vote)
...
@router.get('/results/{election_id}', status_code=HTTPStatus.OK, response_model=ElectionResultsSchema)
async def get_election_results(election_id: int, session: Session):
    ...
    if not tally:
        raise HTTPException(status_code=HTTPStatus.NOT_FOUND, detail='No votes found for this election')
    ...
    for ec in candidates_list:
        ...
        if votes_list:
            ...
            for vote in votes_list[1:]:
                candidate_tally = crypto_service.add_vote_to_tally(candidate_tally, vote.encrypted_vote)
        ...
        candidate = await session.scalar(select(Candidate).where(Candidate.id == ec.fk_candidate))
